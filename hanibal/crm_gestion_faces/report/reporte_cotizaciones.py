...
class reporte_cotizacion(report_sxw.rml_parse):
    
            
    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(reporte_cotizacion, self).__init__(cr, uid, name, context=context)
    ...
    def _inicio(self,data):
        ...
        user_id = False
        if data.get('form', False) and data['form'].get('user_id', False): 
        ...
        print partner_id , user_id , product_id,desde,hasta
        
        var_sub = ' '
        
        if partner_id:
            var_sub = var_sub + ' and so.partner_id = {0}'.format(partner_id)
        if user_id:
            var_sub = var_sub + ' and crm.user_id = {0}'.format(user_id)
        if product_id:
            var_sub = var_sub + ' and sol.product_id = {0}'.format(product_id)
        if desde and hasta:
            var_sub = var_sub + " and so.date_order between '{0}' and '{1}' ".format(desde,hasta)
        elif desde:
            var_sub = var_sub + " and so.date_order >= '{0}' ".format(desde)
        elif hasta:
            var_sub = var_sub + " and so.date_order <= '{0}' ".format(hasta)
        
        var = ' '
        fin = """ group by saol.product_id,pp.name_template
        order by 1 """
        if partner_id:
            var = var + ' and so.partner_id = {0}'.format(partner_id)
        if user_id:
            var = var + ' and crm.user_id = {0}'.format(user_id)
        if product_id:
            var = var + ' and saol.product_id = {0}'.format(product_id)
        if desde and hasta:
            var = var + " and so.date_order between '{0}' and '{1}' ".format(desde,hasta)
        elif desde:
            var = var + " and so.date_order >= '{0}' ".format(desde)
        elif hasta:
            var = var + " and so.date_order <= '{0}' ".format(hasta)

                
        query = """
        select pp.name_template as producto , 
        
        (    select  count(*) ::numeric from 
            crm_lead   as cl , sale_order as so , sale_order_line as sol , product_product as pp 
            where cl.type = 'opportunity'
            and cl.partner_id = so.partner_id
            and pp.id = sol.product_id
            and sol.product_id = saol.product_id
            and so.id = sol.order_id
            and tipo in ('ini','uni') {2}    ) 
            /
            (select count(*) ::numeric
            from (select  sol.id from 
            crm_lead   as cl , sale_order as so , sale_order_line as sol 
            where cl.type = 'opportunity'
            and cl.partner_id = so.partner_id
         ...
         from sale_order so
        join crm_lead as crm on crm.partner_id = so.partner_id
        join sale_order_line as saol on saol.order_id = so.id
        join product_product as pp on saol.product_id = pp.id
        where crm.type = 'opportunity'
        and so.tipo in ('ini','uni')
        {0} 
        {1}
        """.format(var, fin , var_sub)
...
report_sxw.report_sxw(
    'report.reporte.cotizacion',
    'rt.genera.reporte',
    'addons/crm_gestion_faces/report/reporte_cotizaciones.rml',
    parser=reporte_cotizacion,header=False)
