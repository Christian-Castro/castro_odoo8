...
class account_move_line(osv.osv):
    ...
    alumno_id = fields.Many2one('res.partner',related='move_id.alumno_id', store=True, readonly=True, copy=False)
    jornada_id = fields.Many2one('jornada',related='move_id.jornada_id', store=True, readonly=True, copy=False)
    
    seccion_id = fields.Many2one('seccion',related='move_id.seccion_id', store=True, readonly=True, copy=False)
    curso_id = fields.Many2one('curso',related='move_id.curso_id', store=True, readonly=True, copy=False)
    paralelo_id = fields.Many2one('paralelo',related='move_id.paralelo_id', store=True, readonly=True, copy=False)
    jornada_id = fields.Many2one('jornada',related='move_id.jornada_id', store=True, readonly=True, copy=False)
    ...
    mes = fields.Selection( (('1','Enero'),
                               ...
                               ('10','Octubre'),
                               ('11','Noviembre'),
                               ('12','Diciembre')) , 'Mes', required=False,related='move_id.mes')
    anio = fields.Selection( (_calculo_anios) , 'Año', required=False,store=True,related='move_id.anio')
...
class account_journal_cobro(models.Model):
    ...
    def _accesos_usuarios_1(self):
        ...
        for l in self:
            ...
            #     'usuarios_ids':lista_usuario
            #     })
        print(l.usuarios_ids,'l.usuarios_ids')
