...
class NotasCreditoCliente(models.Model):
    ...
    factura = fields.Many2one('account.invoice',domain=[('id','=',0)])#,domain="[('state','=','open'),('type','=','out_invoice'),('residual','!=',0 )]"
    description = fields.Char(required=False)
    establecimiento = fields.Char(size=3)
    punto_emision = fields.Char(size=3)
                    ...
                    if line.quantity > total_resta:
                        raise except_orm(_('Error!'), _(
                            "La cantidad de {0} es mayor a la cantidad restante. (Quedan {1})".format(
                                line.product_id.name, int(total_resta))))
