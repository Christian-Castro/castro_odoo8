...
from openerp.addons.resource.faces.pcalendar import _default_calendar
from pdftools.pdfdefs import null
...
class sale_order(osv.osv):
    ...
    _columns = {
        ...
        #ARGNESIS
        #----------------------------------------------------------------------
        'estado_informativo':fields.char('Estado informativo',size=200),
        'novedades':fields.text('Novedades'),
        'date_confirma':fields.char('Fecha Confirma'),
        'date_crea':fields.char('Fecha Creacion'),
        'fecha_entrega':fields.date('Fecha entrega'),
        'tipo_cot':fields.selection( (('COT_SALES','Ventas anteriores'),('COT_CRM','Ventas nuevas')),'Tipo de ventas' ),
        #----------------------------------------------------------------------
    }
    ...
    def create(self, cr, uid, vals, context=None):
        if context is None:
            context = {}
        #arnegsis
        if not self.pool.get('res.partner').browse(cr, uid, [vals['partner_id']] ).vat:
            if self.pool.get('res.partner').browse(cr, uid, [vals['partner_id']] ).customer != True :
                #MENSAJE A MOSTRAr PARA LOC CONTACTOS EN PETICIONES
                raise Warning('No puede crear peticiones con un nuevo contacto!')
        #arnegsis
        if vals.get('name', '/') == '/' and vals['tipo_cot'] == 'COT_SALES':
            print 'ESTA ACA  ORIGINAL'
            vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'sale.order') or '/'
        if vals.get('name', '/') == '/' and vals['tipo_cot'] == 'COT_CRM':
            print 'ESTA ACAC EN FALSO PARA OTRO TIPO DE COTIAZACION '
            vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'journal_sale_vou' ) or '/'
    ...
    def print_order_sales(self, cr, uid, ids, context=None):
            return self.pool['report'].get_action(cr, uid, ids, 'cotizacion.venta', context=context)

    def print_order_crm(self, cr, uid, ids, context=None):
            return self.pool['report'].get_action(cr, uid, ids, 'cotizacion.venta.crm', context=context)
    ...
    def action_button_confirm(self, cr, uid, ids, context=None):
        ...
        if not so.partner_id.vat:
            raise osv.except_osv('Alerta',"No puede generar pedidos con el cliente "+str(so.partner_id.name)+" !")  
        if so.tipo_cot == 'COT_CRM':
            raise osv.except_osv('Alerta',"No puede confirmar el pedido actual !")  

        if not so.client_order_ref:
            raise osv.except_osv('Alerta',"Antes de confirmar su venta por favor ingrese informació en el campo referencia del cliente!")
        for l in so.order_line:
            if l.product_uom_qty > l.product_id.cantidad:
                raise osv.except_osv('Alerta',"El producto -"+str(l.product_id.name)+"- no dispone de suficiente stock. ¡Disponibles"+str(l.product_id.cantidad)+"!")
        #--------------------------------------------------------------------

        assert len(ids) == 1, 'This option should only be used for a single id at a time.'
        self.signal_workflow(cr, uid, ids, 'order_confirm')

        #SI TODO ESTA BIEN PASA A BORRAR Y MIGRAR
        cr.execute("delete from web_side_ven_pfa_g where pfac_idweb = '"+str(so.id)+"' ")
        cr.execute("delete from web_side_ven_pdfa_g where pdfa_idweb = '"+str(so.id)+"' ")
        cr.commit()
        d = datetime.strptime( str((so.write_date)[:10]) ,"%Y-%m-%d")
        date_confirma = str(d.strftime("%d"))+'/'+str(d.strftime("%m"))+'/'+str(d.strftime("%Y"))
        date_confirmarp = str(d.strftime("%Y"))+'-'+str(d.strftime("%m"))+'-'+str(d.strftime("%d"))
        d = datetime.strptime( str((so.date_order)[:10]) ,"%Y-%m-%d")
        date_crea = str(d.strftime("%d"))+'/'+str(d.strftime("%m"))+'/'+str(d.strftime("%Y"))
        so.write({'date_confirma':date_confirmarp})
        #--------------------------------------------------------------------
        #--------------------------INSERTAR----------------------------------
        insert = """INSERT INTO web_side_ven_pfa_g(
            pfac_codcia, pfac_coddiv, pfac_fecha, pfac_codpre, pfac_hecpor, 
            pfac_totito, pfac_subtot, pfac_subtotd, pfac_total, pfac_obs, 
            pfac_nulo, pfac_fecnul, pfac_codcli, pfac_codage, pfac_plazo, 
            pfac_fecven, pfac_estado, pfac_dirent, pfac_ordcom, pfac_cerrado, 
            pfac_codweb, pfac_idweb,pfac_migrado,pfac_estadoinformativo,pfac_fechaconfirmaweb,
            pfac_migraestado,pfac_coddircli,pfac_codinv)
        VALUES (%s, %s, %s, %s, %s, 
            %s, %s, %s, %s, %s, 
            %s, %s, %s, %s, %s, 
            %s, %s, %s, %s, %s, 
            %s, %s,%s, %s,%s, %s, %s, %s);"""
        if so.partner_id.street2 == False :
            cod = None
        else:
            cod = str(so.partner_id.street2)
        cr.execute(insert,('DW','DW',str(date_crea),so.pricelist_id.default_code,so.user_id.name,
                                round(float(so.amount_tax),2),round(float(so.amount_untaxed),2),'0.00',round(float(so.amount_total),2),so.note or '',
                                '','',so.partner_id.vat,so.user_id.codigo,'',
                                '',so.state,self.cnv(so.partner_id.street),so.client_order_ref,'N',
                                self.cnv(so.name),so.id,'N','',str(date_confirma),'', cod , 'PT' )
                            )
        for l in so.order_line:
            for t in l.tax_id:
                imp = 0.0
                if str(t.amount) == '0.12':
                    imp = float(l.price_subtotal * 0.12)
                if str(t.amount) == '0.0':
                    imp = 0.0
            insert2=""" INSERT INTO web_side_ven_pdfa_g(
                        pdfa_codcia, pdfa_coddiv, pdfa_codart, pdfa_codunimed, pdfa_can, 
                        pdfa_precio, pdfa_total, pdfa_porito, pdfa_monitouni, pdfa_monitotot, 
                        pdfa_preciomod, pfac_codweb, pdfa_idweb,pdfa_idwebline,pdfa_migrado,pdfa_codinv)
                        VALUES (%s, %s, %s, %s, %s,    
                        %s, %s, %s, %s, %s, 
                        %s, %s, %s, %s,%s,%s); """
            cr.execute(insert2,(  'DW','DW',l.product_id.default_code, l.product_id.uom_id.name,
            l.product_uom_qty,float(l.price_unit),
            l.price_subtotal,round(t.amount,2),round(imp,2),
            float(l.price_subtotal),l.price_unit
            ,so.name,so.id,l.id,'N','PT')   )
        
#         for a in so.order_line:
#             cnt = int(a.product_id.product_tmpl_id.cantidad) - int(a.product_uom_qty)
#             cr.execute("update product_template set cantidad = %s where id = %s ",
#                         (str(cnt),a.product_id.product_tmpl_id.id)) 
   ...
   #     }
    ...
    def action_quotation_send(self, cr, uid, ids, context=None):
        ...
        try:
            ...
            compose_form_id = False
        print 'llega aca antiguuuuuuuuuaaaaaaaaaaaaa' 
        ctx = dict()
        ctx.update({
            'default_model': 'sale.order',
            'default_res_id': ids[0],
    ...
    def action_quotation_send_crm(self, cr, uid, ids, context=None):
        '''
        This function opens a window to compose an email, with the edi sale template message loaded by default
        '''
        print 'moderna**************'
...
class sale_order_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        #CAMBIO 25-03-2018        
        tipo = context.get('tipo_cot','False')
        ...
        # ESTA VALIDACION ES PARA LOS TIPOS DE PRODUCTOS
        print tipo,'ESTE ES EL TIPO DE COT '
        print product_obj.categ_id.id
        if tipo != 'COT_CRM':
            if product_obj.categ_id.id not in (1,):
                raise osv.except_osv(_('No permitido!'), _('El producto no es Vikan.'))
        if tipo == 'COT_CRM':
            if product_obj.categ_id.id not in (1,3):
                raise osv.except_osv(_('No permitido!'), _('El producto no tiene categoria aceptada.'))
        uom2 = False
        if uom:
        ...
        # ARNEGSIS 25-08
        #if product_obj.cantidad <= 0 :
        #    warn_msg = ('Disponible en stock: 0.0 para este producto !')
        #    warning_msgs += _("No stock ! : ") + warn_msg +"\n\n"
